...
socket.on('user joined', (data) => {
  log(data.username + ' joined');
  addParticipantsMessage(data);
  updateDropdownUserList(data);
});

// Whenever the server emits 'user left', log it in the chat body and update user list
socket.on('user left', (data) => {
  log(data.username + ' left');
  addParticipantsMessage(data);
  removeChatTyping(data);
...
function updateDropdownUserList(data) {
  const $userListDropdown = $('#userList');
  $userListDropdown.empty();  // Clear previous users
